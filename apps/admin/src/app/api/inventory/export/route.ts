...
/**
 * Phase 46 — Next proxy for the API inventory CSV export. Mirrors
 * the other four proxies exactly; only the upstream path changes.
 */
export async function GET(request: NextRequest): Promise<Response> {
  const searchParams = request.nextUrl.searchParams;
  const apiPath = `/v1/admin/inventory/export.csv?${searchParams.toString()}`;
